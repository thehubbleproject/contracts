...
contract MerkleTreeUtils {
  ...
  function setDefaultHashes(uint256 depth) internal {
    ...
    for (uint256 i = 1; i < depth; i++) {
      defaultHashes[i] = keccak256(abi.encode(defaultHashes[i - 1], defaultHashes[i - 1]));
    }
  }

  function getZeroRoot() public view returns (bytes32) {
    return keccak256(abi.encode(defaultHashes[MAX_DEPTH - 1], defaultHashes[MAX_DEPTH - 1]));
  }
  ...
  function getDefaultHashAtLevel(uint256 index) public view returns (bytes32) {
    return defaultHashes[index];
  }
   ...
   * NOTE: This is a stateless operation
   */
  function getMerkleRootFromLeaves(bytes32[] memory nodes) public view returns (bytes32) {
  ...
  function verify(
    ...
    // First compute the leaf node
    bytes32 calculatedRoot = computeInclusionProofRoot(_dataBlock, _path, _siblings);
  ...
  function verifyLeaf(
    ...
    bytes32 calculatedRoot = computeInclusionProofRootWithLeaf(_leaf, _path, _siblings);
    return calculatedRoot == _root;
  }
  ...
  function pathToIndex(uint256 path, uint256 height) public pure returns (uint256) {
    uint256 result = 0;
  ...
  function calculateRootTruncated(bytes32[] memory buf) public view returns (bytes32) {
    ...
    while (true) {
      ...
      if (odd == 1) {
        buf[i] = keccak256(abi.encode(buf[i << 1], defaultHashes[level]));
      }
      if (n == 1) {
        break;
      }
  ...
  // function genRoot(bytes calldata _data, uint256 sliceLen) external returns (bytes32) {
  //   uint256 batchSize = _data.length / sliceLen;
  //   require(sliceLen * batchSize == _data.length, "excess data is not expected");

  //   bytes32[] memory buf = new bytes32[](batchSize);
  //   // Hash to leaf
  //   bytes memory data = _data;
  //   uint256 bufOff = 32;
  //   for (uint256 dataOff = 32; dataOff < data.length + 32; dataOff += sliceLen) {
  //     // solium-disable-next-line security/no-inline-assembly
  //     assembly {
  //       mstore(add(buf, bufOff), keccak256(add(data, dataOff), sliceLen))

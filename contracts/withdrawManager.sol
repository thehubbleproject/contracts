...
contract WithdrawManager {
  ...
  constructor(address _registryAddr) public {
    ...
    rollup = Rollup(nameRegistry.getContractDetails(ParamManager.ROLLUP_CORE()));
  ...
  function Withdraw(
    ...
    // require(withdraw_tx_proof._tx.data.toIndex == 0, "Not a withdraw transaction");

    // bool isClaimed = withdrawTxClaimed[_batch_id][withdraw_tx_proof._tx.pathToTx];
    ...
    // address tokenContractAddress = tokenRegistry.registeredTokens(withdraw_tx_proof._tx.data.tokenType);

    // // convert pubkey path to ID
    // uint256 computedID = merkleUtils.pathToIndex(_pda_proof._pda.pathToPubkey, governance.MAX_DEPTH());
    ...
    // address receiver = RollupUtils.calculateAddress(_pda_proof._pda.pubkey_leaf.pubkey);

    // require(
    //   receiver == RollupUtils.HashFromTx(withdraw_tx_proof._tx.data).ecrecovery(withdraw_tx_proof._tx.data.signature),

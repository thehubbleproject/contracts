...
contract Logger {
   ...
   * Rollup Contract *
   ********************/
  event NewBatch(address committer, bytes32 txroot, bytes32 updatedRoot, uint256 index);
  ...
  event BatchRollback(uint256 batch_id, address committer, bytes32 stateRoot, bytes32 txRoot, uint256 stakeSlashed);
  ...
  function logBatchRollback(
    ...
    emit BatchRollback(batch_id, committer, stateRoot, txRoot, stakeSlashed);
  }
  ...
  function logRegisteredToken(uint256 tokenType, address tokenContract) public {
  ...
  function logDepositFinalised(bytes32 depositSubTreeRoot, uint256 pathToSubTree) public {

...
contract DepositManager {
  ...
  modifier onlyRollup() {
    assert(msg.sender == nameRegistry.getContractDetails(ParamManager.ROLLUP_CORE()));
    _;
  }
  ...
  function depositFor(
    ...
    require(tokenContract.transferFrom(msg.sender, address(this), _amount), "token transfer not approved");

    // returns leaf index upon successfull append
    uint256 accID = accountRegistry.register(_pubkey);
    ...
    while (tmp % 2 == 0) {
      ...
      pendingDeposits[pendingDeposits.length - 2] = merkleUtils.getParent(deposits[0], deposits[1]);
      ...
      logger.logDepositLeafMerged(deposits[0], deposits[1], pendingDeposits[0]);
    }
  ...
  function finaliseDeposits(
    ...
    // emit the event
    logger.logDepositFinalised(depositsSubTreeRoot, _zero_account_mp.accountIP.pathToAccount);

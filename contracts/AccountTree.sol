...
contract AccountTree {
  ...
  // (DEPTH) for witness, (1) for leafIndex
  uint256 public constant WITNESS_LENGTH = DEPTH + 1;
  ...
  function _checkInclusion(bytes32 leaf, bytes32[WITNESS_LENGTH] memory witness) internal view returns (bool) {
    require(witness.length == DEPTH, "AccountTree: invalid witness size");
    uint256 leafIndex = uint256(witness[0]);
    uint256 path = leafIndex % SET_SIZE;
    bytes32 acc = leaf;
    for (uint256 i = 1; i < WITNESS_LENGTH; i++) {
      if (path & 1 == 1) {
  ...
  function _checkInclusion(
    bytes32 leaf,
    uint256 leafIndex,
    bytes32[WITNESS_LENGTH - 1] memory witness

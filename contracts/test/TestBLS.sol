...
contract TestBLS {
  ...
  function isOnCurveG1Compressed(uint256 point) external view returns (bool) {
    // return BLS.isOnCurveG1(point & BLS.FIELD_MASK); v0.5 :/
  ...
  function isOnCurveG1CompressedGasCost(uint256[2] calldata point) external returns (uint256) {
    uint256 g = gasleft();
  ...
  function isOnCurveG2CompressedGasCost(uint256[4] calldata point) external returns (uint256) {
    uint256 g = gasleft();
    BLS.isOnCurveG2(point);
    return g - gasleft();
  }

  function isOnCurveG2GasCost(uint256[2] calldata point) external returns (uint256) {
    uint256 g = gasleft();
    BLS.isOnCurveG2(point);
    return g - gasleft();
  }
  ...
  function pubkeyToUncompresed(uint256[2] calldata compressed, uint256[2] calldata y)
    external
    pure
